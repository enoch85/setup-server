...
function install_if_not () {
	if [[ "$(is_this_installed "${1}")" -eq "0" ]]; then
...
function restart_webserver() {
	...
	if which php7.2-fpm > /dev/null; then
...
function ram_check() {
	...
	if [ "${mem_available}" -lt "$((${1}*1002400))" ]; then
...
function network_ok() {
    ...
    if ! service network-manager restart > /dev/null; then
        service networking restart > /dev/null
    fi
	
    sleep 2
	
    if wget -q -T 20 -t 2 http://github.com -O /dev/null & spinner_loading; then
        return 0
    else
        return 1
    fi
...
function calculate_max_children() {
	...
	if [ $PHP_FPM_MAX_CHILDREN -lt 8 ]; then
...
function check_open_port() {
	...
	if [ "$(dpkg-query -s nmap 2> /dev/null | grep -c "ok installed")" == "1" ]; then
		NMAPSTATUS=preinstalled
	fi

	apt update -q4 & spinner_loading
	
	if [ "$NMAPSTATUS" = "preinstalled" ]; then
	...
	if [ "$(nmap -sS -p "$1" "$WANIP4" | grep -c "open")" == "1" ]; then
		...
		if [[ "$(nmap -sS -PN -p "$1" "$2" | grep -m 1 "open" | awk '{print $2}')" = "open" ]]; then
